...
export class BaseElement {
    ...
    public async valitdateAttributeValue(attribute: string, expectedAttribute: string) {
        await this.get().getAttribute(attribute).then((actualAttribute) => {
            if (expectedAttribute === actualAttribute) {
                Logger.log(LogLevel.INFO, `BaseElement: WebElement attribute ${attribute} has the required value ${actualAttribute}`);
            } else {
                Logger.log(LogLevel.ERROR, `BaseElement: WebElement attribute ${attribute} does not have the required value ${actualAttribute}`);
            }
        }).catch(() => {
            Logger.log(LogLevel.ERROR, `BaseElement: Error while find the attribute ${attribute}`);
        });
    }
}
